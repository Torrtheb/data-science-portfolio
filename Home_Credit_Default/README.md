...
The _Home Credit Default Risk_ dataset can be found on Kaggle: (https://www.kaggle.com/competitions/home-credit-default-risk/overview). The raw competition data is not stored in this repository due to size constraints. It can be downloaded directly from Kaggle and placed under 'notebooks_and_initial_tables/tables/' for local experimentation.


## Technologies Used

- **Python**, **NumPy**, **pandas**, **SciPy**, **phik**
- **Matplotlib**, **Seaborn** for EDA and visualisation
- **scikit-learn**, **LightGBM**, **CatBoost**, **HistGradientBoosting**
- **joblib** for model persistence
- **FastAPI** for REST API deployment
- **Docker** for containerisation
- **Google Cloud Run** and **Google Cloud Storage** for hosted inference and artifact storage
- **Streamlit** for interactive, browser-based model exploration
...
For model deployment via a streamlit application, there is an app_memory_optimized.py file, a preprocess_memory_efficient.py file (the same as for model deployment, essentially), the same parquet and model files as for model deployment, a requirements.txt file, and a valid_data csv file which holds data for sending test requests. All files related to streamlit deployment are in the streamlit folder. 

The Streamlit app provides an interactive interface for:

- Scoring a **specific client ID**.
- Scoring a **random client** from the validation population.
- Scoring a **prospective applicant** based on self-reported information.

The app is available at:

https://home-credit-defaultstreamlitapp-memory-o-1alcch.streamlit.app

Usage:

- Select a mode (“Specific Client ID”, “Random Sample”, or “Prospective Applicant”).
- Provide the required inputs.
- Click **Predict Risk**.

The production Streamlit app reads:

- The trained LightGBM model.
- Aggregated bureau / previous-loan features.
- Validation data for sampling client IDs.

from Google Cloud Storage, via a service account configured in *st.secrets*. This keeps the repository clean while still enabling realistic, large-scale inference.
