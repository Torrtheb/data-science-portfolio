...
def train_two_stage_effnetb4(
    ...
    *,
    ...
    use_init_state_splits: bool = False,
    return_state: bool = False,
):
    ...
    Args:
        ...
        init_state: Optional EffnetTrainingState to initialize model weights from.
        use_init_state_splits: If True and init_state is provided, use the train/val/test
            splits from init_state instead of re-splitting from train_label_index.
            If False (default), always re-split from train_label_index to include
            newly pseudo-labeled samples, while still using init_state for weight init.
    ...
    # Decide whether to create new splits or use init_state splits
    create_new_splits = (init_state is None) or (not use_init_state_splits)
    ...
    if create_new_splits:
        ...
        if init_state is not None and not use_init_state_splits:
            # Log that we're using new splits with old weights
            old_train_size = len(init_state.train_idx)
            new_train_size = len(train_idx)
            print(f"Re-splitting with expanded support set:")
            print(f"  Previous train size: {old_train_size}")
            print(f"  New train size:      {new_train_size} (+{new_train_size - old_train_size})")
    else:
        # use_init_state_splits=True: Use exact splits from init_state
        # (ignores any new samples in train_label_index)
        ...
        # Warn if train_label_index has more samples than init_state
        total_in_label_index = sum(len(v) for v in train_label_index.values())
        total_in_init = len(train_idx) + len(val_idx) + len(test_idx)
        if total_in_label_index > total_in_init:
            print(f"⚠️  Warning: train_label_index has {total_in_label_index} samples, "
                  f"but using init_state splits with only {total_in_init}.")
            print(f"    Set use_init_state_splits=False to include new samples.")
