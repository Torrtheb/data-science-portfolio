...
def find_slots_tool(
    ...
    """Return owner‑local open slots for a given day and duration.

    Args:
        day: Owner‑local day spec (e.g., "YYYY‑MM‑DD", "today", "tomorrow").
        duration_minutes: Desired slot length in minutes.
        config: Runnable configuration containing the owner id in
            'configurable.user_id' or 'configurable.owner_id'.

    Returns:
        'ToolFindSlotsOut' containing a list of slot objects with
        'start_local' and 'end_local' in owner‑local time.

    Raises:
        ToolException: If the owner id is missing or the operation fails.
    """
    ...
    try:
        ...
        with SessionLocal() as db:
            ...
            # Choose grid: prefer 30-minute grid for common durations; otherwise 15-minute grid
            ...
            for ws, we in merged:
                ...
                # Slide by grid and include any slot fully inside the merged window
                while cur + dur <= we:
                    out.append(
                        {
...
def create_appointment_tool(
    ...
    """Book an appointment if the requested window is available.

    Enforces identity by requiring at least one of 'person_id',
    'client_email', or 'client_name'. Validates availability inside the
    owner's openings and attaches identity to the appointment.
    ...
    Args:
        ...
        service_option_id: Optional pricing context.
        config: Runnable configuration providing the owner id.

    Returns:
        'ToolCreateApptOut' with ids and UTC timestamps.

    Raises:
        ToolException: For missing owner, identity, invalid model columns, or
    ...
    try:
        ...
        with SessionLocal() as db:
            ...
            # Enforce: require a contact email before creating
...
def update_appointment_details_tool(
    ...
    """Attach or correct the identity for an existing appointment.

    Optionally updates note and price override. Guarantees that identity fields
    are denormalized onto the appointment after success.
    ...
    Args:
        ...
        client_email: Email to resolve/create a person when needed.
        client_name: Exact client name to resolve when no 'person_id'.
        note: Optional owner‑private note to set.
        price_override_cents: Price override for this appointment only.
        config: Runnable configuration providing the owner id.

    Returns:
        'ToolUpdateApptDetailsOut' reflecting attached identity.

    Raises:
        ToolException: If the appointment or person is not found, or on other
        validation errors.
    """
    try:
...
def cancel_appointment_tool(
    ...
    Args:
        ...
        start_local: Owner‑local start time (alternative to 'appointment_id').
        duration_minutes: Duration used with 'start_local' to resolve the appt.
        reason: Optional cancellation note.
        config: Runnable configuration providing the owner id.

    Returns:
        'ToolCancelAppointmentOut' with the canceled status.

    Raises:
        ToolException: If owner or appointment is not found, or inputs invalid.
    """
    log.info(
...
def update_appointment_tool(
    ...
    Only sets fields that exist on the 'Appointment' model, keeping the tool
    safe across schema variations. Handles wallet/bundle bookkeeping and refund
    semantics where applicable.

    Args:
        appointment_id: Target appointment id.
        private_note: Owner‑private note.
        attendance: One of 'unknown|attended|late|no_show'.
        late_minutes: Minutes late if 'attendance='late'.
        payment_status: One of 'unpaid|paid|refunded|waived'.
        amount_paid_cents: Amount the client paid.
        price_override_cents: Per‑appointment price override.
        bundle_id: Attach/detach bundle or wallet.
        apply_wallet_now: If true, auto‑apply wallet funds.
        restore_wallet_now: If true, restore previously applied wallet funds.
        config: Runnable configuration providing the owner id.

    Returns:
        'ToolUpdateAppointmentOut' with an 'updated' diff map.

    Raises:
        ToolException: If the appointment is not found or validation fails.
    """
    try:
        owner_id = _owner_id_from_config(config)
        with SessionLocal() as db:
            ...
            # ------- Ledger helpers (consume/restore one credit) -------
            def _net_for_bundle(bid: Optional[int]) -> int:
                if not bid:
                    return 0
                net = (
            ...
            def _consume_one(bid: int):
                bundle = db.get(PrepaidBundle, bid)
                if not bundle:
                    raise ToolException("Bundle not found")
                if int(bundle.remaining_credits) <= 0:
                    raise ToolException("No remaining credits in bundle")
                bundle.remaining_credits = int(bundle.remaining_credits) - 1
                db.add(bundle)
                db.add(
                    PrepaidLedger(
                        bundle_id=bid,
                        ...
                        amount_cents=0,
                        appointment_id=appt.id,
                        note="Auto-consume on payment (agent)",
                    )
                )
            ...
            # ------- Monetary store-credit helpers (wallets) -------
            ...
            # --- If bundle changed and this appt had net -1 on OLD bundle, restore there ---
            if bundle_changed and prev_bundle_id:
                if _net_for_bundle(prev_bundle_id) == -1:
                    _restore_one(
                        prev_bundle_id, "Auto-restore on bundle change (agent)"
                    )
                _restore_amount(
            ...
            # --- Explicit wallet actions (no payment_status change required) ---
            ...
            if became_unpaid and getattr(appt, "bundle_id", None):
                if _net_for_bundle(appt.bundle_id) == -1:
                    _restore_one(
                        appt.bundle_id,
                        f"Auto-restore on status={new_payment_status} (agent)",
                    )
                _restore_amount(
                    appt.bundle_id,
...
def get_next_appointment_for_client_tool(
    ...
    """Return the earliest upcoming appointment matching a client query.

    Args:
        client_query: Name/email substring to match.
        include_canceled: Whether to include canceled appointments.
        config: Runnable configuration providing the owner id.

    Returns:
        'ToolGetNextApptOut' with 'found' flag and optional appointment.

    Raises:
        ToolException: If the owner is not found or the operation fails.
    """
    try:
        owner_id = _owner_id_from_config(config)
        with SessionLocal() as db:
...
def list_appointments_tool(
    ...
    Args:
        day: Day spec (e.g., "YYYY‑MM‑DD", "today").
        include_canceled: Include canceled rows if True.
        client_query: Optional free‑text filter on client fields/notes.
        config: Runnable configuration providing the owner id.

    Returns:
        'ToolListAppointmentsOut' with normalized owner‑local fields.
    """
    owner_id = _owner_id_from_config(config)
    with SessionLocal() as db:
        owner: Optional[User] = db.query(User).filter(User.id == owner_id).first()
...
def get_appointment_details_tool(
    ...
    """Return authoritative appointment details including identity.

    Use 'appointment_id' when possible; otherwise resolve using
    'start_local' and 'duration_minutes'.

    Args:
        appointment_id: Target appointment id.
        start_local: Owner‑local start time used to resolve when no id.
        duration_minutes: Duration used with 'start_local'.
        config: Runnable configuration providing the owner id.

    Returns:
        'ToolGetAppointmentDetailsOut' with denormalized identity fields.

    Raises:
        ToolException: If the owner or appointment cannot be found.
    """
    owner_id = _owner_id_from_config(config)
    with SessionLocal() as db:
        owner: Optional[User] = db.query(User).filter(User.id == owner_id).first()
...
def reschedule_appointment_tool(
    ...
    Prevents conflicts unless 'allow_override=True'. On conflict, raises a
    'ToolException' with a 'NO_AVAILABILITY:{...}' payload so the agent can
    suggest alternatives.

    Args:
        appointment_id: Target appointment id.
        start_local: New owner‑local start time ('YYYY‑MM‑DDTHH:MM').
        duration_minutes: New duration in minutes.
        allow_override: If True, bypass conflict checks.
        message: Optional message to include in notifications.
        config: Runnable configuration providing the owner id.

    Returns:
        'ToolRescheduleApptOut' with updated UTC times and status.

    Raises:
        ToolException: If owner/appointment not found or bad inputs/conflicts.
    """
    owner_id = _owner_id_from_config(config)
    with SessionLocal() as db:
        owner: Optional[User] = db.query(User).filter(User.id == owner_id).first()
...
def list_post_appointment_actions_tool(
    ...
    """Return a checklist of recently completed appointments likely needing updates.

    Useful for post‑visit follow‑up on attendance and payments.

    Args:
        config: Runnable configuration providing the owner id.

    Returns:
        'ToolListPostApptActionsOut' with items requiring attention.
    """
    owner_id = _owner_id_from_config(config)
    with SessionLocal() as db:
        owner: Optional[User] = db.query(User).filter(User.id == owner_id).first()
...
def help_appointment_updates_tool() -> str:
    """Show how to update appointment details via available tools."""
    lines = [
